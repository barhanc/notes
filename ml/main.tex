...
\subsection{Uogólnione modele addytywne}
...
# Assume we are given a dataset (*$\mc{X} = \{y_i(\bm{x}_i)\}_{i=1}^N$*).
...
while not converged:
	for (*$j$*) in range(1, (*$N$*)+1):
		# Use a smoothing operator such as cubic spline 
		(*$f_j$*) = Smooth((*$\left\{y_i - \beta_0 - \sum_{k \neq j} f_k(\bm{x}_i^k)\right\}_{i=1}^N$*))

		# Mean centering of estimated functions
		(*$f_j$*) = lambda (*$x$*): (*$f_j(x) - 1/N \sum_{i=1}^N f_j(\bm{x}_i^j)$*)
