...
\begin{enumerate}[label=\Roman*.]
    ...
    \begin{itemize}
        ...
        czym musi zachodzić \(\oper{P}_1\oper{P}_2 = \oper{\mathsf{0}}\) (istotnie wówczas
        \((\oper{P}_1 + \oper{P}_2)^2 = \oper{P}_1 + \oper{P}_2 \)).
    ...
    \end{quote}
    natomiast operator rzutowy \(\oper{1}-\oper{P}_A(\lambda)\): 
    \begin{quote}
        czy wielkość fizyczna \(A\) ma wartość większą od \(\lambda\)?
    \end{quote}
    ...
    \end{equation*}
    Wartość oczekiwaną funkcji \(f\) wielkości \(A\) dla układu reprezentowanego przez \(\Psi\)
    obliczamy jako
    \begin{equation*}
        \langle f(A) \rangle = \bra{\Psi}f(\oper{A})\ket{\Psi}\,.
    \end{equation*}
    ...
    tworzonym wedle określonych reguł w danej realizacji teorii kwantów.
    
    \item \textbf{O kolapsie.} Po pomiarze układu w stanie \(\ket{\Psi}\) zwracającego odpowiedź na
    pytanie \(\oper{P}\) stan układu zostaje natychmiastowo zredukowany do
    \begin{equation*}
        \ket{\Psi} \to \frac{\oper{P}\ket{\Psi}}{\sqrt{\bra{\Psi}\oper{P}\ket{\Psi}}}
    \end{equation*}
    jeśli pomiar dał odpowiedź pozytywną albo do
    \begin{equation*}
        \ket{\Psi} \to \frac{(\oper{1}-\oper{P})\ket{\Psi}}{\sqrt{1 - \bra{\Psi}\oper{P}\ket{\Psi}}}
    \end{equation*}
...
\begin{itemize}
    ...
    \item \(\oper{X}^2 = \oper{Y}^2 = \oper{Z}^2 = \oper{1}\)
    ...
    \begin{equation*}
        \begin{split}
            ...
            &= (\mathbf{a}\cdot\mathbf{b})\oper{1} + \im(\mathbf{a}\times\mathbf{b})\cdot\boldsymbol{\sigma}\,.
        \end{split}
    \end{equation*}

    \item Wielkość \(\exp(\im\mathbf{a}\cdot\boldsymbol{\sigma})\) wynosi
    \begin{equation*}
            \exp(\im\mathbf{a}\cdot\boldsymbol{\sigma}) = \oper{1} \cos{|\mathbf{a}|} + \im\left(\frac{\mathbf{a}}{|\mathbf{a}|}\cdot\boldsymbol{\sigma}\right)\sin{|\mathbf{a}|}\,.
    \end{equation*}
...
\begin{equation*}
    \begin{cases}
        ...
        \beta(t)  = \left(c_1 m_+\e^{+\im\Omega t} + c_2 m_-\e^{-\im\Omega t}\right)\e^{-\im\omega t / 2} 
    \end{cases}\quad,
\end{equation*}
gdzie
\begin{equation*}
    m_\pm = \pm\sqrt{n^2 + \left(1 - \frac{\omega}{2\omega_0}\right)^2} - \left(1 - \frac{\omega}{2\omega_0}\right)
\end{equation*}
oraz
\begin{equation*}
    \Omega = \sqrt{n^2\omega_0^2 + \left(\omega_0 - \frac{\omega}{2}\right)^2}\,.
\end{equation*}
