...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Prawo odbicia}\\
        Promień padający, odbity i normalna do powierzchni padania leżą w jednej płaszczyźnie, a kąt padania jest równy kątowi odbicia.
    }%
}
\medskip
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        ...
        Promień padający, załamany i normalna do powierzchni padania leżą w jednej płaszczyźnie, a stosunek sinusa kąta padania do sinusa kąta załamania jest równy względnemu współczynnikowi załamania światła
        \begin{equation*}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        ...
        Promień świetlny poruszający się w danym ośrodku między punktami \(A\), \(B\) ma tę własność, że droga optyczna
        \begin{equation*}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        ...
        Całkowita emisja energetyczna \(R=\int_0^\infty R(\lambda)\dd{\lambda}\) (gdzie \(R(\lambda)\) jest mocą na jednostkę powierzchni wypromieniowywaną przez ciało w postaci fali EM o długości z przedziału \((\lambda, \lambda+\dd{\lambda})\)) ciała doskonale czarnego jest wprost proporcjonalna do czwartej potęgi jego temperatury
        \begin{equation*}
            R=\sigma T^4\,.
        \end{equation*}
    }%
