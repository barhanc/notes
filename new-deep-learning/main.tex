...
\begin{equation*}
    \begin{split}
        &\mathbf{X}^{ij} = \mathbf{x}_j^i\,,\quad \mathbf{X} = \mqty[\mathbf{x}_1 & \ldots & \mathbf{x_n}]\\
        &\mathbf{T}^{ij} = [y_j = c_i]\\
        &\boldsymbol{\Phi}^{ij}(\mathbf{X}; \mathbf{W}) = \boldsymbol{\phi}^i(\mathbf{x}_j;\mathbf{W}) = \sum_{k=1}^d \mathbf{W}^{ik}\mathbf{X}^{kj}\,,\quad \boldsymbol{\Phi}(\mathbf{X};\mathbf{W}) = \mathbf{W}\mathbf{X}\\
        &\boldsymbol{\Pi}^{ij}(\boldsymbol{\Phi}) = \boldsymbol{\pi}^i\left(\boldsymbol{\phi}(\mathbf{x}_j;\mathbf{W})\right) = \frac{\exp\boldsymbol{\Phi}^{ij}}{\sum_{k=1}^K \exp \boldsymbol{\Phi}^{kj}}
    \end{split}
\end{equation*}
gdzie każda kolumna macierzy \(\mathbf{X}\) jest wektorem cech danego przykładu; każda kolumna
\(\mathbf{T}\) jest tzw. \textbf{wektorem one--hot} dla danego przykładu tj. wektorem binarnym, w
którym dokładnie na jednej pozycji jest wartość 1 i pozycja ta odpowiada prawidłowej klasie dla
danego przykładu; każda kolumna macierzy \(\boldsymbol{\Phi}\) jest wektorem \textbf{mlogitów} tj.
liczb rzeczywistych, które po zastosowaniu funkcji softmax dają wartości prawdopodobieństwa każdej
klasy; możemy zapisać
\begin{equation}\boxed{
    L(\mathcal{X}; \mathbf{W}) = -\sum_{j=1}^n\sum_{i=1}^K \mathbf{T}^{ij} \log \left[ \boldsymbol{\Pi}^{ij}(\mathbf{\Phi}(\mathbf{X}; \mathbf{W})) \right]\,.
}\end{equation}
...
jeszcze wzór na pochodną funkcji kosztu po parametrach \(\mathbf{W}\). Obliczmy najpierw pochodną
\(L\) po \(\boldsymbol{\Phi}^{ij}\) (dla przejrzystości zapisu nie piszemy granic sumowania --
wynikają one naturalnie z wymiarów macierzy)
\begin{equation}
    \pdv{L}{\boldsymbol{\Phi}^{pq}} = -\sum_{i,j,r,s} \frac{\mathbf{T}^{ij}}{\boldsymbol{\Pi}^{rs}}\delta_{ir}\delta_{js}\pdv{\boldsymbol{\Pi}^{rs}}{\boldsymbol{\Phi}^{pq}} = - \sum_{i,j} \frac{\mathbf{T}^{ij}}{\boldsymbol{\Pi}^{ij}}\pdv{\boldsymbol{\Pi}^{ij}}{\boldsymbol{\Phi}^{pq}}\,.
\end{equation}
Jednocześnie
\begin{equation}
    \pdv{\boldsymbol{\Pi}^{ij}}{\boldsymbol{\Phi}^{pq}} = \frac{\delta_{pi}\delta_{qj}\exp\boldsymbol{\Phi}^{ij}\left[\sum_k \exp\boldsymbol{\Phi}^{kj}\right] - \delta_{qj} \exp\boldsymbol{\Phi}^{ij} \exp\boldsymbol{\Phi}^{pj}}{\left[\sum_k \exp\boldsymbol{\Phi}^{kj}\right]^2}\,,
\end{equation}
skąd
\begin{equation}
    \frac{1}{\boldsymbol{\Pi}^{ij}}\pdv{\boldsymbol{\Pi}^{ij}}{\boldsymbol{\Phi}^{pq}} = \delta_{pi}\delta_{qj} - \delta_{qj}\boldsymbol{\Pi}^{pj}\,.
\end{equation}
Z powyższego zatem
\begin{equation}
    \pdv{L}{\boldsymbol{\Phi}^{pq}} = -\sum_{i,j} \mathbf{T}^{ij}\delta_{pi}\delta_{qj} + \sum_{i,j}\mathbf{T}^{ij}\delta_{qj}\boldsymbol{\Pi}^{pj} = \boldsymbol{\Pi}^{pq} - \mathbf{T}^{pq}\,,
\end{equation}
gdzie skorzystaliśmy z faktu, iż z konstrukcji dla dowolnej kolumny \(q\) macierzy \(\mathbf{T}\)
zachodzi \(\sum_{i}\mathbf{T}^{iq} = 1\). Możemy zapisać powyższy wzór w eleganckiej postaci
macierzowej
\begin{equation}\boxed{
...
\begin{equation}
    \pdv{L}{\mathbf{W}^{rs}} = \sum_{p,q}\pdv{L}{\boldsymbol{\Phi}^{pq}}\pdv{\boldsymbol{\Phi}^{pq}}{\mathbf{W}^{rs}}\,,
\end{equation}
gdzie
\begin{equation}
    \pdv{\boldsymbol{\Phi}^{pq}}{\mathbf{W}^{rs}} = \sum_t \delta_{rp} \delta_{st} \mathbf{X}^{tq} = \delta_{rp}\mathbf{X}^{sq}\,,
\end{equation}
skąd
\begin{equation}
    \pdv{L}{\mathbf{W}^{rs}} = \sum_{q} (\boldsymbol{\Pi}^{rq} - \mathbf{T}^{rq}) \mathbf{X}^{sq}\,,
\end{equation}
co również możemy zapisać w zwartej postaci macierzowej
\begin{equation}\boxed{
    \pdv{L}{\mathbf{W}} = (\boldsymbol{\Pi} - \mathbf{T}) \mathbf{X}^T\,.
}\end{equation}
...
\begin{equation*}
    \begin{split}
        \forall A \subset \mathbb{R}^n :\, &P(\mathbf{X}_t \in A \mid \mathbf{X}_{t-1} = \mathbf{x}_{t-1}, \ldots, \mathbf{X}_0 = \mathbf{x}_0) \\
        &= P(\mathbf{X}_t \in A \mid \mathbf{X}_{t-1} = \mathbf{x}_{t-1})\,.
    \end{split}
\end{equation*}
