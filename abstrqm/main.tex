...
% * Some stuff I forgor: show that [A,B] = iC where C is hermitian, 
% * Rotations SU(2), SO(3)
% * Adiabatic theorem
%
...
\footnote{W mechanice kwantowej wielkość \(\boldsymbol{\mu} = \frac{1}{2}g\mu_\text{B}\mathbf{S}\)
jest spinowym momentem magnetycznym cząstki kwantowej. W powyższym wzorze \(g\) to bezwymiarowy
czynnik, a \(\mu_\text{B}\) to stała fizyczna zwana \textit{magnetonem Bohra}.}, której składowe
\(S_x\), \(S_y\), \(S_z\) są w przestrzeni \(\mathscr{H}\) reprezentowane przez samosprzężone
operatory Pauliego \(\oper{X}\), \(\oper{Y}\), \(\oper{Z}\). Istotnie zgodnie z twierdzeniem
Ehrenfesta
\begin{equation*}
