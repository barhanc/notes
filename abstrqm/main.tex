...
% * Rewrite the abstract theory to deal explicitly only with finite dimensional case (?). [Based on
%   IBB] 
...
Hilberta -- zarówno takich, w których możemy znaleźć nieskończoną ale przeliczalną bazę ortonormalną
(np. przestrzeń Hilberta rzeczywistych funkcji nieparzystych na odcinku \([-L;L]\) spełniających
warunki Dirichleta, w której \(f(x) = \sum_{i=1}^\infty a_i\sin\frac{i\pi x}{L}\)), jak i w
przypadku występującym np. w najprostszym zagadnieniu nierelatywistycznej cząstki swobodnej
poruszającej się w jednym wymiarze, której funkcja falowa jest dana w postaci
\begin{equation*}
    \Psi(x,t) = \frac{1}{\sqrt{2\pi}}\int\limits_{-\infty}^{+\infty}\tilde{\Psi}(k)\e^{\im(kx - \omega t)}\dd{k}
\end{equation*}
gdzie wektor \(\Psi(x,t)\) należący do przestrzeni Hilberta funkcji całkowalnych z kwadratem modułu
jest wyrażony jako ciągła superpozycja wektorów ,,bazowych'' \(\e^{\im(kx - \omega t)}\) (które nie
należą w ogóle do tej przestrzeni) ze współczynnikami danymi transformatą Fouriera \(\tilde{\Psi}(k)
= \mathcal{F}\{\Psi(x,0)\}\).
