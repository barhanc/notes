...
&\bm{\Sigma}^{ij} = \bm{\sigma}^j(\bm{\phi}(\bm{x}_i; \bm{W})) = \frac{\exp(\bm{\Phi}^{ij})}{\sum_{l=1}^k \exp(\bm{\Phi}^{il})}
\end{split}\quad,
\]
...
\[
L(\mc{X}; \bm{W}) = - \sum_{i=1}^n \sum_{j=1}^k \bm{T}^{ij} \log \bm{\Sigma}^{ij}\,,
\]
skąd
\[
\pdv{L}{\bm{\Phi}^{ab}} = \sum_{c,d} \pdv{L}{\bm{\Sigma}^{cd}}\pdv{\bm{\Sigma}^{cd}}{\bm{\Phi}^{ab}}\,,
\]
gdzie
\[
\pdv{L}{\bm{\Sigma}^{cd}} = - \sum_{i,j} \frac{\bm{T}^{ij}}{\bm{\Sigma}^{ij}} \delta_{ic} \delta_{jd} = -\frac{\bm{T}^{cd}}{\bm{\Sigma}^{cd}}
\]
oraz
\[
\begin{split}
\pdv{\bm{\Sigma}^{cd}}{\bm{\Phi}^{ab}} &= \exp(\bm{\Phi}^{cd}) \frac{ \delta_{ac} \delta_{bd} \left[\sum_l \exp(\bm{\Phi}^{cl})\right] - \delta_{ac} \exp(\bm{\Phi}^{cb})}{\left[\sum_l \exp(\bm{\Phi}^{cl})\right]^2}\\
                                       &= \bm{\Sigma}^{cd} \delta_{ac} \delta_{bd} - \delta_{ac} \bm{\Sigma}^{cd} \bm{\Sigma}^{cb} 
\end{split}\quad.
\]
Z powyższego
\[
\pdv{L}{\bm{\Phi}^{ab}} = \sum_{c,d} \frac{\bm{T}^{cd}}{\bm{\Sigma}^{cd}} \left( \delta_{ac} \bm{\Sigma}^{cd} \bm{\Sigma}^{cb} - \bm{\Sigma}^{cd} \delta_{ac} \delta_{bd} \right) = \bm{\Sigma}^{ab} - \bm{T}^{ab}\,,
\]
...
\[
\pdv{L}{\bm{W}^{ab}} = \sum_{c,d} \pdv{L}{\bm{\Phi}^{cd}} \pdv{\bm{\Phi}^{cd}}{\bm{W}^{ab}} = \sum_c \left(\bm{\Sigma}^{ca} - \bm{T}^{ca}\right) \bm{X}^{cb} \,,
\]
co możemy zapisać w zwartej postaci macierzowej
\[
\pdv{L}{\bm{W}} = \left(\bm{\Sigma} - \bm{T}\right)^T \bm{X}\,.
\]
...
% \section{Nautral Language Processing}
