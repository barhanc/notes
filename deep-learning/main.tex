...
Bernoulliego z parametrem \(p(\bm{x}; \bm{w})\) (prawdopodobieństwem klasy pozytywnej, zwanym
również funkcją wiążącą, z ang. \textit{link function}) zależnym od estymowanych parametrów \(\bm{w}\)
\[
t(\bm{x}) \mid \bm{w} \sim \mathrm{Ber}(p(\bm{x}; \bm{w}))\,.
\]
W przypadku regresji logistycznej jako funkcję \(p(\bm{x}; \bm{w})\) przyjmiemy
...
\[
\mc{L}(\mc{X}; \bm{w}) = \prod_{i=1}^n p(\bm{x}_i;\bm{w})^{t_i} (1 - p(\bm{x}_i;\bm{w}))^{1 - t_i}\,,
\]
skąd funkcja kosztu
\[
\boxed{
\begin{split}
L(\mc{X}; \bm{w}) &= - \sum_{i=1}^n \left[ t_i \log p(\bm{x}_i; \bm{w}) + (1 - t_i) \log (1 - p(\bm{x}_i; \bm{w})) \right]\\
