...
\subsubsection{Regularyzacja normą wektora wag}

Możemy oczywiście dodać standardowy człon regularyzujący do funkcji kosztu, który nakłada
ograniczenia na wartości poszczególnych wag, tak jak robiliśmy my to dla płytkich modeli uczenia
maszynowego. W takim podejściu jednak każda waga jest ograniczana w sposób niezależny od pozostałych
wag. Amplituda wagi nie jest więc wprost powiązana z amplitudami innych wag w sieci. Metodą
regularyzacji, która wymusza konkurencję pomiędzy wagami, tak aby ważne wagi rosły kosztem wag
nieistotnych jest regularyzacja normą wektora wag, która wprowadza limit na normę wektora wag.


\subsubsection{Dropout}

Dobrym sposobem regularyzacji metod uczenia maszynowego jest uśrednianie odpowiedzi wielu modeli
...
sposobów budowy modelu zespołowego jest \emph{bagging} (od bootstrap aggregating). Oczywiście w
...
pojedynczej sieci neuronowej jest dropout. dropout polegający na tym, iż w procesie treningu ze z
góry zadanym prawdopodobieństwem ,,wyłączamy'' wyjście neuronu (tj. dla każdej aktualizacji wartości
parametrów sieci, w algorytmie wstecznej propagacji błędu mnożymy wyjście \(\bm{A}_i\) przez tensor
\(\bm{P}\), którego każdy element może być równy \(0\) z prawdopodobieństwem \(p\) lub 1 z
prawdopodobieństwem \(1-p\)). Mechanizmu dropout używamy tylko w procesie treningu. W przypadku
inferencji wyjście neuronu jest z kolei mnożone przez następujący stosunek
\begin{equation*}
...
dość dużych stałych uczących oraz współczynników pędu, a także trenować znacznie większe i głębsze
sieci. Okazuje się, iż w praktyce takie duże, mocno trenowane sieci z regularyzacją uczą się lepiej
niż mniejsze, które by tej regularyzacji nie potrzebowały. Jeśli sieć nie przeucza na zbiorze
trenującym to najpewniej lepsze wyniki uzyskamy ucząc większą sieć (która normalnie mogłaby
przeuczyć) i stosując dropout.
