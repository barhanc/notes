...
przez tensor \(\bm{P}\), którego każdy element może być równy \(0\) z prawdopodobieństwem \(p\) lub
1 z prawdopodobieństwem \(1-p\)). Mechanizmu dropout używamy tylko w procesie treningu. W przypadku
inferencji wyjście neuronu jest z kolei mnożone przez następujący stosunek
\[
    1 - \frac{\text{liczba wyłączeń neuronu}}{\text{liczba aktualizacji parametrów sieci}}\,.
\]
