...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[>={Stealth[black]}, 
            ...
            \path [->] (X) edge node {$X_{jk}$} (Ein1);
            \path [->] (W) edge node {$W_{ij}$} (Ein1); 
            \path [->] (B) edge node {$B_{i}$} (Ein2); 
            \path [->] (I) edge node {$I_k$} (Ein2);

            \path [->] (Ein1) edge node {$E^{1}_{ik} := W_{ij}X_{jk}$} (Add1); 
            \path [->] (Ein2) edge node {$E^{2}_{ik} := B_iI_k$} (Add1);

            \path [->] (Add1) edge node {$A_{ik} := E^{1}_{ik} + E^{2}_{ik} $} (sigma); 
            \path [->] (sigma) edge node {$Y_{ik} := \sigma(A_{ik})$} (Add2);
            \path [->] (Ein2) edge[bend left=60] node {$E^{2}_{ik} := B_iI_k$} (Add2);
            \path [->] (Add2) edge[bend left=80] node {$F_{ik} := E_{ik}^2 + Y_{ik}$} (F);
        ...
        \begin{scope}[>={Stealth[black]}, 
            ...
            % \path [<-] (sigma) edge node {$Y_{ik} := \sigma(A_{ik})$} (Add2);
            % \path [<-] (Ein2) edge[bend left=60] node {$E^{2}_{ik} := B_iI_k$} (Add2);
            \path [<-] (Add2) edge[bend right=80] node {$\delta_{pq,rs}$} (F);

        \end{scope}
    \end{tikzpicture}    
\end{figure}

Chcemy obliczyć pochodne postaci \(\partial F_{pq} / \partial X_{rs}\), \(\partial F_{pq} / \partial
W_{rs}\), \(\partial F_{pq} / \partial B_{r}\), \(\partial F_{pq} / \partial I_{r}\), czyli pochodne
po liściach naszego grafu obliczeń.
