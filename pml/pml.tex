...
\begin{equation*}
    T_{x_1,..,x_s} \leftarrow X_{i_1,..,i_p}Y_{j_1,..,j_q}Z_{k_1,..,k_r}\cdots
\end{equation*}
...
poprzez sumę iloczynów odpowiednich elementów \(A, B\). Dodatkowo dla funkcji \(f: \mathbb{R}
\mapsto \mathbb{R}\) zapis postaci \(Y_{ijk..} = f(X_{ijk..})\) dla dowolnego tensora \(X\)
definiuje tensor \(Y\), którego wartości są wartościami funkcji \(f\) dla odpowiednich wartości
tensora \(X\). Dodatkowo jeśli \(f: \mathbb{R} \mapsto \mathbb{R}\), a \(F : \mathbb{R}^{\sh_1}
\mapsto \mathbb{R}^{\sh_2}\) to przez zapis \(Y = (f \circ F)(X)\) będziemy rozumieć 
\begin{equation*}
    Y_{ijk..} = f(F_{ijk..}(X))\,.
\end{equation*}
...
\begin{tcolorbox}[title=Algorytm wstecznej propagacji błędu]
    \begin{enumerate}
        ...
        \textit{forward propagation}) sieci MLP i zapamiętaj wartości \(Y^{h-1}\) oraz
        \(L^h\).
