...
będziemy oznaczać przez \(\mathbb{R}^{n_1\times..\times n_k} \equiv \mathbb{R}^\sh\).
Odwzorowanie \(F\) o argumentach i wartościach tensorowych zdefiniujemy jako \(F: \mathbb{R}^{\sh_1}
\mapsto \mathbb{R}^{\sh_2}\). Wówczas wartość tego odwzorowania dla argumentu \(X\) (\(\sh(X) =
\sh_1\)) tj. \(F(X)\) jest tensorem (\(\sh(F(X)) = \sh_2\)). Ponadto będziemy utożsamiać zapis
...
\begin{equation*}
    \pdv{(X_{a:n:p,b:m:q,c:l:r})_{ijk}}{X_{xyz}} = \delta_{a+ip,x}\delta_{b+jq,y}\delta_{c+kr,z}\,.
\end{equation*}
