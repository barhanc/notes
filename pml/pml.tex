...
\subsubsection{Entropia względna}\label{sec:entropy}
...
\subsection{Wnioskowanie Bayesowskie}\label{sec:bayesian_inference}
...
\begin{itemize}
    \item wczytywania danych (np. plików w formacie csv)
    
    \item czyszczenia i imputacji danych (usunięcie silnie skorelowanych cech, usunięcie oczywistych
    outlierów lub cech posiadających zbyt dużo brakujących wartości, uzupełnienie wartości cech)
    
    \item wstępnej analizy/eksploracji danych (wykresy, histogramy, correlation heatmap)
    
    \item podział zbioru danych na zbiór treningowy i testowy

    \item skalowanie zmiennych numerycznych (parametry potrzebne do skalowania są obliczane jedynie
    na zbiorze treningowym, aby uniknąć tzw. \textit{data leakage}). Wydzieliliśmy dane testowe po
    to, żeby sprawdzać, jak model poradzi sobie z danymi, których do tej pory nigdy nie widział, bo
    to właśnie takie dane będzie on dostawać w praktyce, po wdrożeniu do realnego systemu. Ta ocena
    obejmuje też etap preprocessingu, w tym skalowania. Więc jeśli etap preprocessingu zobaczy dane
    testowe, to nie będziemy w stanie uczciwie estymować jego zachowania na nowych danych.
    Wykorzystanie danych testowych w procesie treningu to błąd wycieku danych. Skutkuje on
    niepoprawnym, nadmiernie optymistycznym oszacowaniem jakości modelu.

    \item one-hot encoding zmiennych kategorycznych (należy pamiętać, że kodując zmienną
    kategoryczną o \(N\) wartościach używamy \(N-1\) \textit{dummy variables}, aby nie wpaść w tzw.
    \textit{dummy variable trap} -- zależność liniową między cechami)

    \item transformacja logarytmiczna zmiennej objaśnianej -- jeśli histogram wartości zmiennej
    objaśnianej nie przypomina rozkładu normalnego (jest asymetryczny, skośny) to przekształcamy
    wartości tej zmiennej $y \mapsto \log(y)$. Dodatkowa korzyść z takiej transformacji jest taka,
    że regresja liniowa przewiduje dowolne wartości rzeczywiste. Po przekształceniu logarytmicznym
    jest to całkowicie ok, natomiast w oryginalnej przestrzeni trzeba by wymusić przewidywanie tylko
    wartości pozytywnych.

    \item feature engineering -- dodanie wielomianów cech do naszych danych lub skonstruowanie innych danych
...
\begin{itemize}
    \item pełne przeszukiwanie (grid search) - definiujemy możliwe wartości dla różnych hiperparametrów, a
    metoda sprawdza ich wszystkie możliwe kombinacje (czyli siatkę),

    \item losowe przeszukiwanie (randomized search) - definiujemy możliwe wartości jak w pełnym
...
zbiór walidacyjny (validation set). Zbiór taki wycina się ze zbioru treningowego. Dzielimy zatem
nasze dane nie na dwie, ale trzy części: treningową, walidacyjną i testową. Typowe proporcje to
60-20-20\% lub 80-10-10\%.
...
samej ilości danych. Taką metodą jest walidacja skrośna (cross-validation, CV). Polega na tym, że
dzielimy zbiór na $K$ równych podzbiorów, tzw. foldów. Każdy podzbiór po kolei staje się zbiorem
walidacyjnym, a pozostałe łączymy w zbiór treningowy. Trenujemy zatem \(K\) modeli dla tego samego
zestawu hiperparametrów i każdy testujemy na zbiorze walidacyjnym. Mamy \(K\) wyników dla zbiorów
walidacyjnych, które możemy uśrednić (i ew. obliczyć odchylenie standardowe). Takie wyniki są
znacznie bardziej wiarygodne.
...
\subsection{Metryki do oceny klasyfikacji}

W przypadku regresji ocena jakości modelu sprawdzała się do użycia jednej z metryk RMSE, MAE. W
przypadku klasyfikacji, a zwłaszcza klasyfikacji silnie niezbalansowanej ocena jakości klasyfikatora
jest zadaniem trudniejszym. Rozpatrujemy tutaj jedynie  klasyfikatory binarne. Dla danego zestawu
cech odpowiedź klasyfikatora może być jedną z czterech:
\begin{itemize}
    \item \textit{True Positive} -- poprawnie zaklasyfikowaliśmy klasę pozytywną jako pozytywną
    \item \textit{True Negative} -- poprawnie zaklasyfikowaliśmy klasę negatywną jako negatywną
    \item \textit{False Positive} -- niepoprawnie zaklasyfikowaliśmy klasę negatywną jako pozytywną
    \item \textit{False Negative} -- niepoprawnie zaklasyfikowaliśmy klasę pozytywną jako negatywną
\end{itemize}
Na podstawie ilości TP, TN, FP i FN na zbiorze testowym możemy wprowadzić następujące podstawowe
metryki
\begin{itemize}
    \item \textit{precision} -- mówi nam o tym jak pewnie klasyfikator znajduje klasę pozytywną
    \begin{equation*}
        \text{precision} = \frac{\text{TP}}{\text{TP}+\text{FP}}
    \end{equation*}
    \item \textit{recall} -- mówi nam o tym jak dobrze wykrywamy klasę pozytywną
    \begin{equation*}
        \text{recall} = \frac{\text{TP}}{\text{TP} + \text{FN}}
    \end{equation*}
\end{itemize}
Zauważmy, że metryki te mają zasadniczy problem, jeśli chcielibyśmy używać tylko jednej z nich:
klasyfikator, który zawsze zwraca klasę pozytywną ma maksymalny recall, a klasyfikator, który zawsze
zwraca klasę negatywną ma nieskończony precision. Nie możemy zatem używać tych miar osobno, tj.
dobry klasyfikator powinien mieć wysokie zarówno precision, jak i recall. Miarą, która łączy
precision i recall jest F1-score zdefiniowany jako średnia harmoniczna precision i recall
\begin{equation*}
    F_1 = \frac{2}{\text{precision}^{-1} + \text{recall}^{-1}}\,.
\end{equation*}
Metryka ta przyjmuje wartości od 0 do 1 i im jej wartość większa tym lepszy klasyfikator. Zauważmy
bowiem, iż aby F1-score był wysoki zarówno precision, jak i recall muszą mieć wartość bliską 1.

Czasami bywa tak, iż każda z czterech możliwości klasyfikatora ma realny koszt (np. przy predykcji,
czy klient przestanie używać naszych usług) i wtedy powinniśmy używać takiej empirycznej funkcji
kosztu (patrz: \ref{sec:bayesian_inference}).

W przypadku klasyfikacji niezbalansowanej często na wytrenowanym już klasyfikatorze dokonujemy tzw.
\textit{threshold tuning}. Normalnie w przypadku klasyfikatora, który zwraca prawdopodobieństwa klas
wybiera klasę o największym prawdopodobieństwie (czyli w przypadku klasyfikacji binarnej o
prawdopodobieństwie większym od 0.5), threshold tuning polega na obniżeniu tego progu. Pytanie jak
wybrać ten próg. Wykorzystujemy do tego celu krzywą ROC (z ang. \textit{Receiver Operating
Characteristic curve}) czyli wykres parametryczny recall i FPR (\textit{False Positive Rate}, czyli
recall dla klasy negatywnej) dla różnych progów klasyfikacji i wybieramy próg, dla którego F1-score
jest maksymalny. Krzywa ROC pozwala dodatkowo wprowadzić jeszcze jedną miarę oceny klasyfikatora,
która jest niezależna od przyjętego progu, tzw. AUROC (z ang. \textit{Area under ROC}), czyli pole
powierzchni pod wykresem krzywej ROC. Miara ta przyjmuje wartości z zakresu [0;1] i im jest większa
tym lepszy mamy klasyfikator. AUROC jest dobrą miarą jakości klasyfikatora (tj. dobrze różnicuje
modele) jeśli mamy klasyfikację umiarkowanie niezbalansowaną. W przypadku klasyfikacji ekstremalnie
niezbalansowanej używamy podobnej miary od AUROC zwanej AUPRC (z ang. \textit{Area under
Precision-Recall Curve}) zdefiniowanej jako pole powierzchni pod wykresem krzywej PRC, czyli krzywej
parametrycznej precision i recall dla różnych wartości progu klasyfikacji.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{\textit{Mutual Information}}. Obliczamy informację wzajemną (patrz:
        \ref{sec:entropy}) i na jej podstawie usuwamy cechy.
    \end{itemize}
...
Ideą redukcji wymiarowości jest rzutowanie cech (z ang. \textit{feature projection}) na nową
przestrzeń o mniejszej liczbie wymiarów. W przeciwieństwie do selekcji cech, zamiast usuwać cechy
tworzymy ich kombinacje (liniowe lub nieliniowe) i w taki sposób otrzymujemy podprzestrzeń realnie
ważną dla problemu. Zaletą takiego podejścia jest to, iż rozważa złożone przekształcenia całej
przestrzeni cech, ale z tego względu jest mniej interpretowalne od metod selekcji cech.

Najbardziej typowe są liniowe metody redukcji wymiarowości w tym przede wszystkim SVD (z ang.
\textit{Singular Value Decomposition}) i PCA (z ang. \textit{Principal Components Analysis}). W
przypadku podejść nieliniowych najpopularniejsze metody to UMAP (z ang. \textit{Uniform Manifold
Approximation and Projection}) oraz t-SNE (z ang. \textit{t-distributed Stochastic Neighbor
Embedding}). 
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        \begin{scope}[every node/.style={circle,thick,draw}]
            \node (X)     at (0,0)  {\(X\)}; \node (W) at (-3,0) {\(W\)}; \node (B) at (-6,0)
            {\(B\)}; \node (I)     at (-9,0) {\(I\)}; \node (Ein1)  at (-1.5,2.5) {Ein}; \node
            (Ein2)  at (-7.5,2.5) {Ein}; \node (Add1)  at (-4.5, 5) {Add}; \node (sigma) at (-4.5,
            7.5) {\(\sigma\)}; \node (Add2)  at (-4.5, 10) {Add}; \node (Id)    at (-4.5,
            12.5){\(F\)};

        \end{scope}
        
        \begin{scope}[>={Stealth[black]}, every node/.style={rectangle,fill=white}, every
            edge/.style={draw=red,very thick}] \path [->] (X) edge node {$X_{jk}$} (Ein1); \path
            [->] (W) edge node {$W_{ij}$} (Ein1); \path [->] (B) edge node {$B_{i}$} (Ein2); \path
            [->] (I) edge node {$I_k$} (Ein2);

            \path [->] (Ein1) edge node {$E^{1}_{ik} \leftarrow W_{ij}X_{jk}$} (Add1); \path [->]
            (Ein2) edge node {$E^{2}_{ik} \leftarrow B_iI_k$} (Add1);

            \path [->] (Add1) edge node {$L^1_{ik} \leftarrow E^{1}_{ik} + E^{2}_{ik} $} (sigma);
            \path [->] (sigma) edge node {$Y_{ik} \leftarrow \sigma(L^1_{ik})$} (Add2); \path [->]
            (Ein2) edge[bend left=60] node {$E^{2}_{ik} \leftarrow B_iI_k$} (Add2); \path [->]
            (Add2) edge node {$L^2_{ik} \leftarrow E_{ik}^2 + Y_{ik}$} (Id);

        \end{scope}
    \end{tikzpicture}    
\end{figure}

Chcemy obliczyć pochodne postaci \(\partial F_{pq} / \partial X_{ab}\), \(\partial F_{pq} / \partial
W_{ab}\), \(\partial F_{pq} / \partial B_{a}\), \(\partial F_{pq} / \partial I_{a}\), czyli pochodne
po liściach naszego grafu obliczeń. W tym celu idąc top-down tj. zaczynając od wierzchołka \(F\)
przekazujemy ziarno do każdej krawędzi wchodzącej do tego wierzchołka, przy czym ziarno obliczamy
rekurencyjnie jako iloczyn (w sensie notacji sumacyjnej Einsteina) ziarna w danym wierzchołku i
pochodnej wyrażenia w tym wierzchołku po wyrażeniu w wierzchołku, z którego wychodzi dana krawędź.
Dochodząc do liścia inkrementujemy ziarno pamiętane w liściu o przychodzącą wartość. Otrzymana
wartość ziarna w wierzchołku będzie równa pochodnej funkcji wyjściowej po tym parametrze. Rozpatrzmy
przykład.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[>={Stealth[black]}, every node/.style={rectangle,fill=white}, every
            edge/.style={draw=blue,very thick}] \path [<-] (X) edge node {$s_{pqab} \leftarrow
            s_{pqxy} \pdv{E_{xy}^1}{X_{ab}}$} (Ein1); \path [<-] (W) edge node {$s_{pqab} \leftarrow
            s_{pqxy} \pdv{E_{xy}^1}{W_{ab}}$} (Ein1); \path [<-] (B) edge[bend right=80] node
            {$s_{pqa} \leftarrow s_{pqtu}\pdv{E_{tu}^2}{B_{a}}$} (Ein2); \path [<-] (B) edge node
            {$s_{pqa} \leftarrow s_{pqxy}\pdv{E_{xy}^2}{B_{a}}$} (Ein2); \path [<-] (I) edge node
            {$s_{pqa} \leftarrow s_{pqxy}\pdv{E_{xy}^2}{I_{a}}$} (Ein2); \path [<-] (I) edge[bend
            left=80] node {$s_{pqa} \leftarrow s_{pqtu}\pdv{E_{tu}^2}{I_{a}}$} (Ein2); \path [<-]
            (Ein1) edge node {$s_{pqxy}\leftarrow s_{pqvw}\pdv{L_{vw}^1}{E_{xy}^1}$} (Add1); \path
            [<-] (Ein2) edge node {$s_{pqxy}\leftarrow s_{pqvw}\pdv{L_{vw}^1}{E_{xy}^2}$} (Add1);
            \path [<-] (Add1) edge node {$s_{pqvw} \leftarrow s_{pqtu}\pdv{Y_{tu}}{L_{vw}^1}$}
            (sigma); \path [<-] (sigma) edge node {$s_{pqtu} \leftarrow s_{pqrs}
            \pdv{L_{rs}^2}{Y_{tu}}$} (Add2); \path [<-] (Ein2) edge[bend left=60] node {$s_{pqtu}
            \leftarrow s_{pqrs}\pdv{L_{rs}^2}{E_{tu}^2}$} (Add2); \path [<-] (Add2) edge node
            {$s_{pqrs} \leftarrow \pdv{F_{pq}}{L_{rs}^2}$} (Id);
