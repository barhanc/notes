...
\KwIn{Zbiór punktów $\mc{X} = \{x_1, x_2, \ldots, x_n\}$, liczba klastrów $k$}
\KwOut{Centroidy $C = \{c_1, \ldots, c_k\}$ oraz przypisanie punktów do klastrów}

Wybierz losowo $k$ punktów z $X$ jako początkowe centroidy $C = \{c_1, \ldots, c_k\}$\;

\Repeat{iteration limit}{
    \ForEach{$x_i \in X$}{
        Przypisz $x_i$ do najbliższego centroidu:
        \[
        \text{cluster}(x_i) = \argmin_{j \in \{1,\ldots,k\}} \norm{x_i - c_j}^2
        \]
    }
    \ForEach{$c_j \in C$}{
        Oblicz nową pozycję centroidu jako średnią punktów przypisanych do klastra $j$:
        \[
...
\KwIn{Zbiór punktów $\mc{X} = \{x_1, x_2, \ldots, x_n\}$}
\KwOut{Drzewo dendrogramowe opisujące proces łączenia klastrów}

\BlankLine
\textbf{Inicjalizacja:} Utwórz $n$ jednoelementowych klastrów $\mathcal{C} = \{\{x_1\}, \{x_2\}, \ldots, \{x_n\}\}$\;

\While{liczba klastrów w $\mathcal{C}$ > 1}{
    Oblicz macierz odległości $D(C_i, C_j)$ dla wszystkich $C_i, C_j \in \mathcal{C}$, $i \ne j$\;

    Znajdź parę klastrów $(C_p, C_q)$ taką, że
    \[
    (C_p, C_q) = \arg\min_{C_i \ne C_j} D(C_i, C_j)
    \]

    Połącz $C_p$ i $C_q$ w nowy klaster $C_{new} = C_p \cup C_q$\;
    Zaktualizuj $\mathcal{C} := \mathcal{C} \setminus \{C_p, C_q\} \cup \{C_{new}\}$\;

    Dodaj $(C_p, C_q, D(C_p, C_q))$ do struktury dendrogramu\;
}
...
% TODO: Przedziały ufności, testowanie hipotez
% TODO: Conformal prediction, uncertainty estimation

% Jednym z istotnych zagadnień, często traktowanych pobieżnie jest estymacja poziomu niepewności
% modelu. W przypadku zagadnień klasyfikacji, gdy mamy do czynienia z modelem probabilistycznym,
% typowo na jego wyjściu mamy liczby, które możemy interpretować jako prawdopodobieństwa danych
% klas. Problemem jest jednak ich kalibracja, tzn. jeśli model dla danego zbioru wektorów cech
% zwraca prawdopodobieństwo klasy pozytywnej równe np. 60%, to czy rzeczywiście około 3/5 z tych
% przykładów ma klasę pozytywną. W przypadku regresji natomiast prostym sposobem na estymację
% poziomu niepewności jest wykorzystanie regresji kwantylowej, tj. dopasowanie trzech modeli
% estymujących odpowiednio medianę i np. kwantyl 0.05 i 0.95 aby otrzymać obszar do którego powinno
% wpadać 90% przypadków. W przypadku regresji kwantylowej problem jest jednak taki sam, skąd mamy
% pewność, że uzyskane w taki sposób oszacowania są skalibrowane, a nie zawyżają/zaniżają
% niepewność. Oba te problemy można rozwiązać odpowienio kalibrując nasz model na dodatkowym
