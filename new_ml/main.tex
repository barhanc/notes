...
\begin{equation*}
    \mathbb{KL}(p , q) = \int\limits_{\mathbb{R}^n} p(\mathbf{x})\log\frac{p(\mathbf{x})}{q(\mathbf{x})} \dd[n]{\mathbf{x}}\,,
\end{equation*}
która określa podobieństwo między dwoma rozkładami prawdopodobieństwa tj. dla ustalonego rozkładu
\(p\) dla wszystkich \(q\) zachodzi \(\mathbb{KL}(p,q) \geq 0\), przy czym równość zachodzi iff \(p
= q\) (ponownie nierówność Jensena).
...
information})
\begin{equation*}
    \mathbb{I}(\mathbf{x} , \mathbf{y}) = \mathbb{KL}(p(\mathbf{x}, \mathbf{y}), p(\mathbf{x})p(\mathbf{y})) = \int\limits_{\mathbb{R}^{2n}} p(\mathbf{x}, \mathbf{y})\log\left\{\frac{p(\mathbf{x}, \mathbf{y})}{p(\mathbf{x})p(\mathbf{y})}\right\} \dd[n]{\mathbf{x}} \dd[n]{\mathbf{y}}\,.
\end{equation*}
...
\begin{equation*}
    \begin{split}
        &\boldsymbol{\theta}(\mathbf{x}; \oper{W}_1, \ldots,\oper{W}_s, \mathbf{b}_1, \ldots, \mathbf{b}_s)\\
        &= \left(\mathbf{g}\circ\mathbf{a}_s\circ \mathbf{f}_{s-1}\circ\mathbf{a}_{s-1}\circ\ldots\circ \mathbf{f}_1\circ\mathbf{a}_1\right)(\mathbf{x})\,,
    \end{split}
\end{equation*}
gdzie 
\begin{equation*}
...
Rozpatrzymy najpierw znajdowanie estymaty punktowej MLE dla sieci MLP. Załóżmy, iż mamy dane
obserwacje iid \(D = \{(y_1, \mathbf{x}_1), \ldots, (y_m, \mathbf{x}_m)\}\). Wiarygodność ma postać
\begin{equation*}
...
oraz
\begin{equation*}
    \pdv{\mathbf{a}_k}{\mathbf{f}_{k-1}} = \oper{W}_k\,,\quad \pdv{\mathbf{a}_k}{\oper{W}_k} = \mqty[\mathbf{f}_{k-1} & 0 & \ldots & 0 \\ 0 & \mathbf{f}_{k-1} & \ldots & 0 \\ \vdots & \vdots & \ddots & \vdots \\ 0 & 0 & \ldots & \mathbf{f}_{k-1}]
\end{equation*}
zatem
\begin{equation*}
