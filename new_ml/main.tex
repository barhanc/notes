...
liniowej funkcja aktywacji neuronu w warstwie wyjściowej to po prostu funkcja identycznosciowa,
...
\end{equation*}
gdzie \(D = (Y_{ik}, X_{jk})\) jest zbiorem danych treningowych. Natomiast w przypadku wieloklasowej
...
\end{equation*}
gdzie tensor \(I\) składa się z samych 1, natomiast \(T_{:,k}\) jest wektorem w postaci
\textit{one-hot encoding} prawidłowej klasy dla wektora zmiennych objaśniających \(X_{:,k}\) ze
...
Niech \(Y_{ab}^h = \sigma(L_{ab}^h)\), wówczas \(L_{ab}^h = W_{ac}^hY_{cb}^{h-1} + B_a^h\), przy
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            \begin{split}
                &\oper{W}_k = \oper{W}_k - \frac{\epsilon}{|D_i|}\pdv{\ell}{\oper{W}_k}\\
                &\mathbf{b}_k = \mathbf{b}_k - \frac{\epsilon}{|D_i|}\pdv{\ell}{\mathbf{b}_k}\\
            \end{split}
        \end{equation*}
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    zbiór testowy (na którym ewalluujemy model po treningu).
